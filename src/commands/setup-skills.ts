...
const SKILL_FOLDER = ".claude/skills/r0";
...
export async function setupSkillsCommand(): Promise<void> {
  const folderPath = path.resolve(process.cwd(), SKILL_FOLDER);
  const skillFile = path.join(folderPath, "SKILL.md");

  if (await exists(skillFile)) {
    console.log(chalk.yellow(`r0 skill already exists at ${SKILL_FOLDER}/SKILL.md`));
    return;
  }

  await mkdir(folderPath, { recursive: true });
  await Bun.write(skillFile, SKILL_TEMPLATE);

  console.log(chalk.green(`Installed r0 skill for Claude Code`));
  console.log();
  console.log(`  ${chalk.bold(".claude/skills/r0/SKILL.md")}`);
  console.log();
  console.log(chalk.dim("The skill adds /r0 command to Claude Code."));
  console.log(chalk.dim("Use it to convert requirements into backlog tasks."));
  console.log();
  console.log(chalk.dim("Example:"));
  console.log(`  ${chalk.cyan("/r0 add user authentication with email/password")}`);
}
